...
[uitest\_run.ps1](XamarinPipelineDemo.UITest/uitest_run.ps1): script to run
...
* You need to upload example.keystore as a secure file.
* You need to create a variable group named `android_demo_var_group`. In it,
...
## Give Each Build An Increasing Version ##
...
James Montemagno "Mobile App Tasks for VSTS"
...
Another confusion you may have is that the AndroidSigning@3 task has an input
named "keystoreAlias" (also "apksignerKeystoreAlias"), but keystores do not
have aliases; keys within keystores have aliases.  You specify the keystore by
the file name, then you specify the key by the key's alias. I have reported
this misnaming as a [problem on Developer
Community](https://developercommunity.visualstudio.com/content/problem/1292515/androidsigning.html).

TODO: when in doubt, use keystore password for key password

TODO: errors of "can't find $(someVariable) secure file" mean the someVariable
is not defined.  Check that you are referencing the appropriate variable group
in your yaml's "variables" section, and check that "someVariable" exactly
matches what you have in your variable group.
