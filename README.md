...
* [Explanation Of The Journey: Deadends, Pitfalls, Solutions](#Explanation-Of-The-Journey-Deadends-Pitfalls-Solutions)
  ...
  * [Publish UI Tests](#Publish-UI-Tests)
...
[`uitest_run.ps1`](LocalScripts/uitest_run.ps1): script to run
...
go into the `LocalScripts` folder and run `uitest_run.ps1`.  You'll get a test
results file `TestResult.xml` and a detailed log `uitest.log` that is useful
for troubleshooting. The script tries to use `adb` and `msbuild` from your
`PATH` environment variable and a few other locations.  You might have to add
your `adb` or `msbuild` directories to your `PATH`. Also, you might have to set
the `ANDROID_HOME` environment variable to something like `C:\Program Files
...
Similarly,
[`uitest_run.ps1`](LocalScripts/uitest_run.ps1) is in a `LocalScripts` folder
...
I wish I could find the place that I first saw the idea of building the UI
tests while waiting for the Android emulator to start.
